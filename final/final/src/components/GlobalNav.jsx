    ...
    return (
      item.hasSubItem ?
        <ul className="global-nav-menu" key={item.name}>
          <li className="menu-item" key={item.name}><a className="header-link" href="/" aria-label={`${item.name} in nav bar`}>{item.name}</a>
            <ul>
              ...
              <li key={item.children[3].name} aria-label='Dropdown item'><a className="menu-item-dropdown" onClick={onNav} href={item.children[3].path}>{item.children[3].name}</a></li>
            </ul>
          </li>
        </ul>
        :
